   ...
   const renderAddCoupon= (req, res, next) => {
    ...
    const addCoupon= async (req, res, next) => {
        try {
            ...
            couponIdExist = await coupon.find({ couponCode: req.body.couponCode }).lean();
   ...
   const validateCoupon=async (req, res, next) => {
       try { 
        ...
        //  if( req.session.coupocid==req.body.couponId){
        //    console.log("fdsa");
        //     return res.json({ message: 'coupona already Used se' });

        //  }
        currentDate = new Date();
        
        if (coupons && coupons.couponLimit>0 )
        // {   req.session.coupocid=coupons.couponCode
            
        { if(couponExist){
                
             return res.json({ message: 'coupona already Used' });    
